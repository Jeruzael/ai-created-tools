...
- Status: Revised
- Revision notes: Improved the Security Check findings review UI with expanded filters, keyboard-selectable rows, richer summaries, and collapsible technical evidence.
...
| v1.14 | 2026-06-18 | Revised | Added first read-only Standard Review collectors and live Security Check finding/detail rendering. |
| v1.15 | 2026-06-18 | Revised | Added scheduled task review, scoped services summary, and command/name review indicators for autorun-style entries. |
| v1.16 | 2026-06-18 | Revised | Added referenced file verification with file existence, Authenticode signature status, SHA-256 hashes, and timestamps. |
| v1.17 | 2026-06-18 | Revised | Added normalized review scoring and structured plain-language explanations for every Security Check finding. |
| v1.18 | 2026-06-18 | Revised | Improved Findings Review UI with category/status/signal filters, keyboard row selection, summary counts, and collapsible technical evidence. |
...
- Add the original v1.12 warning action alert for the placeholder shell, making clear at that time that no registry keys, files, tasks, Defender settings, browser policies, or logs were read.
...
- Connect the Standard Review lifecycle step to real read-only collectors.
- Collect registry startup entries from common current-user and local-machine Run/RunOnce locations without editing the registry.
- Collect current-user and all-users Startup folder entries without deleting, moving, or disabling items.
- Collect Chrome and Edge browser policy registry values and group evidence by browser where available.
- Collect current-user proxy settings with plain-language explanation of normal and review-worthy cases.
- Collect DNS server settings with adapter/interface evidence where Windows exposes it.
- Collect Microsoft Defender exclusions and group evidence by exclusion type.
- Flag broad-looking Defender exclusions as Medium Review while keeping all language as review guidance, not a malware verdict.
- Save collected findings and skipped-source details in local Security Check records.
- Show real findings in the Security Check table with selectable detail rows, technical evidence, and safe next steps.
- Show skipped or unavailable sources as skipped/access-denied/error details instead of failing the whole check.
- Keep scheduled tasks, file verification, event logs, Sysmon, baselines, allowlists, and report downloads for later slices.

Implemented in v1.15:

- Add read-only scheduled task collection through Windows PowerShell.
- Show scheduled task name, path, state, author, principal user, run level, last run time, next run time, last result, actions, and triggers when Windows exposes them.
- Add command review indicators for encoded/hidden PowerShell patterns, script or command-host utilities, shell-launched commands, web URLs, and user-writable or temporary locations.
- Add Windows-like executable name review for autorun commands that look like core Windows component names outside expected Windows system folders.
- Apply command/name indicators to startup registry entries, Startup folder entries, scheduled task actions, and automatic service commands.
- Add a scoped Windows services summary and individual service findings only when automatic service commands have review indicators.
- Keep all findings in `Needs Review`, `Low Review`, or `Medium Review` language and avoid malware verdicts.
- Keep file verification, event logs, Sysmon, baselines, allowlists, and report downloads for later slices.

Implemented in v1.16:

- Add read-only File Verification after Standard Review collectors finish.
- Extract referenced file paths from startup registry entries, Startup folder entries, scheduled task actions, Windows service commands, Defender exclusions, and command evidence when possible.
- Mark missing referenced files as `File Missing`.
- For existing referenced files, collect file existence, size, created time, modified time, SHA-256 hash, Authenticode signature status, signer subject, signer issuer, and certificate dates when available.
- Mark unsigned or problematic signature states as review signals, not malware verdicts.
- Show signature/hash collection failures as `Unavailable` skipped-source details instead of crashing the check.
- Add File Verification to Security Check category summaries and count unsigned verified files in the summary cards.
- Link Security Check users to the existing Verification Guide for signature and SHA-256 review guidance.
- Keep event logs, Sysmon, baselines, allowlists, and report downloads for later slices.

Implemented in v1.17:

- Normalize every Security Check finding score to a 0-100 range.
- Assign score bands consistently: `Info`, `Low Review`, `Medium Review`, and `High Review`.
- Add score explanations that state higher scores mean more suspicious patterns, not proof of malware.
- Add structured plain-language sections to every finding:
  - `What this is`
  - `Why it matters`
  - `Why it was flagged`
  - `What to check next`
  - `What not to do`
- Preserve `Info` and `No Obvious Issue` behavior for known-normal or informational items.
- Update the Security Check detail panel to show plain-language sections separately from technical evidence.
- Keep all language as review guidance, not a final safety or malware verdict.

Implemented in v1.18:

- Expand Security Check summary cards with review status and category counts.
- Add Findings Review filters for search, severity, category, status, signal type, and baseline placeholder.
- Add signal filters for unsigned files, command indicators, and user-writable paths.
- Make finding rows clickable and keyboard-accessible with Enter/Space.
- Highlight the selected finding row.
- Keep long paths, command lines, hashes, registry values, and evidence wrapped in the table and detail panel.
- Add calm empty states for categories with no findings.
- Put technical evidence inside a collapsible detail section separate from the plain-language explanation.
